...
import { detectProjects, readProjectConfig, getScriptRunCmd, getSystemUpdateCmd, formatProjectDetection, formatPackageScripts } from "../../../src/utils/projectDetect.js";
...
describe("detectProjects", () => {
  ...
  it("detects lock file or recognizes monorepo workspace", () => {
    const projects = detectProjects();
    const node = projects.find((p) => p.configFile === "package.json");
    // In a monorepo workspace, lock file may be at root not in package dir
    expect(node?.lockFile === "package-lock.json" || node?.lockFile === undefined || node?.lockFile === "").toBe(true);
  });
...
describe("readProjectConfig", () => {
  ...
  it("lists scripts from package.json", () => {
    ...
    // core package has build+test (dev is in cli package)
  });
