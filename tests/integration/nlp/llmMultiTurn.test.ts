...
describe("LLM multi-turn: unit tests (no LLM needed)", () => {
  ...
  it("near-misses are populated before LLM call", async () => {
    const { classifyMulti } = await import("../../../packages/core/src/nlp/multiClassifier.js");
  ...
  it("semantic similarity provides candidates for vague input", async () => {
    const { findSimilarIntents } = await import("../../../packages/core/src/nlp/semanticSimilarity.js");
  ...
  it("concept expansion helps with synonyms", async () => {
    const { expandQuery } = await import("../../../packages/core/src/nlp/conceptExpansion.js");
