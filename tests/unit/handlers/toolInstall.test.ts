...
describe("pending action for tool.info", () => {
  it("suggestAction can store an install action", async () => {
    const { suggestAction, getLastPendingAction, clearPendingActions } = await import("../../../packages/core/src/conversation/pendingActions.js");
    clearPendingActions();
  ...
  it("isAffirmation recognizes yes/do it/ok", async () => {
    const { isAffirmation } = await import("../../../packages/core/src/conversation/pendingActions.js");
